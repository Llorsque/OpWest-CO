...
export function tryLogin(login,password){
  const a=ACCOUNTS.find(a=>a.login.toLowerCase()===login.trim().toLowerCase()&&a.password===password);
  if(!a)return{ok:false,error:"Ongeldige gebruikersnaam of wachtwoord."};
  const s={login:a.login,naam:a.naam,role:a.role,loginAt:new Date().toISOString()};
  localStorage.setItem(LS_SESSION,JSON.stringify(s)); return{ok:true,session:s};
}
