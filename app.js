...
const ROUTES = {
  "/": Landing, "/verenigingen": Verenigingen, "/trajecten": Trajecten,
  "/projecten": Projecten, "/aankomend": Aankomend, "/gemeente": Gemeente,
...
function updateUserBadge(){
  const el=qs("#userBadge"); if(!el)return;
  el.innerHTML=`<div class="user-badge"><div class="user-badge__avatar">${state.userName.charAt(0).toUpperCase()}</div><div class="user-badge__info"><div class="user-badge__name">${state.userName}</div><div class="user-badge__role">${isAdminUser()?"Admin":"Viewer"}</div></div></div>`;
}
...
function setActiveNav(route){
  ...
  const el=qsa(".nav__item").find(a=>a.getAttribute("href")===`#${route}`);
  el?.classList.add("is-active");
}

async function renderRoute(){
  const route=getRoute(); setActiveNav(route);
  state.isAdmin=isAdminUser()&&isAdmin(); updateAdminBadge();
