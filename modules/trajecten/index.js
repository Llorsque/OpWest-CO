...
export function bind(state,root){
  ...
  root.querySelector("#detailThema")?.addEventListener("change",e=>{const t=state.trajecten.find(x=>x.id===state.ui.selectedTrajectId);if(t)t.thema=e.target.value;});
    ...
    ${items.map(t=>{const isSel=state.ui.selectedTrajectId===t.id;
      return `<div class="trj-row ${isSel?"trj-row--active":""}" data-select-trj="${esc(t.id)}">
        <div class="trj-row__main">
          ...
          <div class="trj-row__sub">${t.thema?`<span class="pill pill--thema" style="font-size:10px;margin-right:4px;">${esc(t.thema)}</span>`:""} ${t.coach?esc(t.coach):""}</div>
        </div>
        <div class="row" style="gap:6px;flex-shrink:0;">
          ...
          ${admin?`<button class="btn--icon-del" data-del-trj="${esc(t.id)}" title="Verwijder">x</button>`:""}
        </div>
      </div>`;}).join("")}
  </div>`;
}
    ...
    <div class="panel__body"><div class="split"><div>
      ...
      <div class="subpanel"><div class="subpanel__header"><strong>Logboek</strong> <span class="muted">(${log.length})</span></div>
        ...
        ${log.length?log.map((e,i)=>`<div style="display:flex;gap:10px;padding:6px 0;border-bottom:1px solid var(--border);">
          ...
          ${admin?`<button class="btn--icon-del" data-del-log="${i}">x</button>`:""}</div>`).join("")
